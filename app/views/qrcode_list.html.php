...
<div class="text-end">
    ...
    <table id="list_qr" class="table table-bordered table-striped text-center">
        ...
        <?php if (!$qrlist): ?>
            <tbody>
                <tr><td colspan="6"><center><i>Vous n'avez pas encore créé de QRCode</i></center></td></tr>
            </tbody>
        <?php else: ?>
            <tbody>
                ...
                <?php foreach($qrlist as $qr): ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($qr->visites): ?>
                            <a href="<?php echo "$urlbase/qrcode/".$qr->user_id."/stats/".($qr->_id ?? $qr->id)."/week" ?>">
                                <?php echo count($qr->visites); ?>
                            </a>
                            <?php endif ?>
                        </td>
                        <td>
                            <a data-bs-toggle="tooltip" title="Modifier" class="p-1 text-dark" href="/qrcode/<?php echo $qr->user_id ?>/edit/<?php echo $qr->_id ?? $qr->id ?>"><i class="bi bi-pencil-fill"></i></a>
                            <a data-bs-toggle="tooltip" title="Visualiser et télécharger" class="p-1 text-dark" href="/qrcode/<?php echo $qr->user_id ?>/parametrage/<?php echo $qr->_id ?? $qr->id ?>"><i class="bi bi-qr-code"></i></a>
                            <a data-bs-toggle="tooltip" title="Voir les statistiques d'utilisations" class="p-1 text-dark" href="/qrcode/<?php echo $qr->user_id ?>/stats/<?php echo $qr->_id ?? $qr->id ?>/week"><i class="bi bi-clipboard2-data"></i></a>
                            <a data-bs-toggle="tooltip" title="Télécharger la fiche d'accompagnement" class="p-1 text-dark" href="/<?php echo $qr->_id ?? $qr->id ?>/fiche"><i class="bi bi-truck"></i></a>
                            <a data-bs-toggle="tooltip" title="Dupliquer (créer un nouveau QR Code à partir des informations de celui-ci)" href="/qrcode/<?php echo $qr->user_id ?>/duplicate/<?php echo $qr->_id ?? $qr->id ?>" class="text-dark float-end"><i class="bi bi-copy"></i></a>
                        </td>
                        <td><input form="multiExportForm" type="checkbox" name="qrcodes[]" value='<?php echo $qr->_id ?? $qr->id; ?>'></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        <?php endif; ?>
